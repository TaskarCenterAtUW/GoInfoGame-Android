...
class EditElementsDao(private val db: Database, val preferences: Preferences? = null) {
    ...
    fun put(id: Long, elementKeys: List<ElementKey>) {
        db.insertMany(
            ...
            elementKeys.map { arrayOf(id, it.type.name, it.id, workspaceId) }
        )
    }

    fun get(id: Long): List<ElementKey> =
        db.query(NAME, where = "$EDIT_ID = $id AND $WORKSPACE_ID = $workspaceId") {
    ...
    fun getAllByElement(elementType: ElementType, elementId: Long): List<Long> =
        db.query(NAME,
            where = "$ELEMENT_TYPE = ? AND $ELEMENT_ID = ? AND $WORKSPACE_ID = ?",
            args = arrayOf(elementType.name, elementId, workspaceId),
