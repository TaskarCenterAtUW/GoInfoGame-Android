...
class TangramPinsSpriteSheet(
    ...
    val sceneUpdates: List<Pair<String, String>> by lazy {
        val isSpriteSheetCurrent = prefs.getInt(Prefs.PIN_SPRITES_VERSION, 0) == com.tcatuw.goinfo.BuildConfig.VERSION_CODE

        val spriteSheet = when {
            !isSpriteSheetCurrent || com.tcatuw.goinfo.BuildConfig.DEBUG || shouldBeUpsideDown() -> createSpritesheet()
    ...
    private fun createSpritesheet(): String {
        ...
        prefs.edit {
            putInt(Prefs.PIN_SPRITES_VERSION, if (shouldBeUpsideDown()) -1 else com.tcatuw.goinfo.BuildConfig.VERSION_CODE)
